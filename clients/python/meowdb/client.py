...
class MeowDBError(Exception):
    pass

class MeowDB:
    """Synchronous Python client for MeowDB."""
    ...
    def _send_line(self, cmd: str) -> str:
        if not cmd.endswith("\r\n"):
            cmd += "\r\n"
        ...
        line = f.readline()
        if not line:
            raise MeowDBError("Connection closed by server")
        trimmed = line.strip()
        if trimmed.startswith("-ERR"):
            raise MeowDBError(trimmed[5:])
...
class AsyncMeowDB:
    """Asynchronous asyncio Python client for MeowDB."""
